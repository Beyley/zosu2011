...
pub const Packet = Packets.Packet(PacketId.channel_leave, struct {
    const Self = @This();

    ///The receive mode the client wants, this could mean sending no one, everyone, or only friends
    channel: Bancho.ArrayString(Bancho.Client.MAX_CHANNEL_LENGTH),
    ...
    pub fn deserialize(reader: Bancho.Client.Reader) !Self {
        return .{ .channel = try Bancho.ArrayString(Bancho.Client.MAX_CHANNEL_LENGTH).deserialize(reader) };
    }
});
